...
def getContractResponse():
    ...
    """ if len(paragraph) != 0 and len(question) != 0:
        print('getting predictions')

        with open('responses.json', 'r') as file:
            file_data = file.read().strip()
            responses_data = json.loads(file_data) if file_data else []

            for partie in responses_data:
                if partie['question'] == question:
                    return partie['answer']
                break
            else:
                print("start of predictions 1")
                predictions = run_prediction([question], paragraph, 'marshmellow77/roberta-base-cuad', n_best_size=1)
                print("start of predictions 2")
        ...
        return "Invalid input. Please select both question and contract file." """
